...
import 'package:imedics_latest/screens/patient_screens/All_appointments/waiting_room/view.dart';
import 'package:imedics_latest/screens/patient_screens/All_appointments/widgets/app_details_screen.dart';
...
Widget appointmentCardWidget(
    ...
    List<PatientAppointmentModel> listOfApp,
    bool isCompleted) {
  UserDocModel doc = isCompleted == true
      ? appointController.getUpComingAppointDoc(
          controller, controller.state.completedAppointmentList[index].docId!)
      : appointController.getUpComingAppointDoc(
          controller, controller.state.patientAppointmentList[index].docId!);
  bool isWaitingRoom = appointController.checkWaitingRoom(listOfApp[index]);
  return Padding(
          ...
          child: Column(
            ...
            children: [
              Text(
                "${doc.name}",
                style:
                    getBoldStyle(color: Colors.black, fontSize: MyFonts.size16),
              ),
              padding5,
              Text(
                "${doc.specialization} ",
                ...
                children: [
                  Row(
                    children: [
                      ...
                      padding5,
                      Text(
                        "${listOfApp[index].selectedTimeSlot}",
                        style: getBoldStyle(
                            color: Colors.black, fontSize: MyFonts.size16),
                      ),
                    ],
                  ),
                  padding5,
              ...
              isCompleted == false
                  ? isWaitingRoom
                      ? CustomButton(
                          ...
                          onPressed: () {
                            Get.to(()=>WaitingRoomScreen(doctor: doc,appoint: listOfApp[index],));

...
class SplashController extends GetxController {
      ...
      () async {
        ...
        if (userId != "") {
          ...
          if (isPatient == true) {
            ...
            await fetchDoctorDetail(userId).then((value) async {
              log('id:$userId');
              bool isIndDoctor = await Prefrences().getDoctorType();
              if (isIndDoctor == true) {
  ...
  Future<void> fetchPatientDetails(String id) async {
    try {
      final response = await http.get(
        Uri.parse('${AppConstants.baseUrl}/getpatient/${id}'),
  ...
  Future<void> fetchDoctorDetail(String id) async {
    ...
    try {
      final response = await http.get(
        Uri.parse('${AppConstants.baseUrl}/getDoctorDetail/${id}'),
        headers: {"Content-Type": "application/json"},
      );
      if (response.statusCode == 200) {
        var data = jsonDecode(response.body);
        ...
        AppConstants.docProfileImgUrl = data['image'];
